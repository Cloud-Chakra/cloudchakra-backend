...
const storageService = {
  async storeData(jsonObj) {
    ...
    const { shards, paddingLength, totalLength } = createShards(jsonStr);
  ...
  async retrieveData(docId) {
    ...
    if (onlineShards.length < 2) {
      throw new Error('Not enough shards online to reconstruct');
    }
    ...
    // Remove padding if it was added during store
    const jsonStr = reconstructedBuffer.toString('utf8');
  ...
  async handlePhoneOffline(deviceId) {
    ...
    for (const doc of docs) {
      ...
      if (lostShard.shardIndex === 2) {
        // Parity shard lost: XOR of two data shards
        ...
        for (let i = 0; i < data0.length; i++) {
          missingShardBuffer[i] = data0[i] ^ data1[i];
        }
      } else if (lostShard.shardIndex === 0) {
        // Data shard 0 lost: XOR of parity and data shard 1
        const parity = otherShardData[otherIndexes.indexOf(2)];
        const data1 = otherShardData[otherIndexes.indexOf(1)];
        missingShardBuffer = Buffer.alloc(parity.length);
        for (let i = 0; i < parity.length; i++) {
          missingShardBuffer[i] = parity[i] ^ data1[i];
        }
      } else {
        // Data shard 1 lost: XOR of parity and data shard 0
        const parity = otherShardData[otherIndexes.indexOf(2)];
        const data0 = otherShardData[otherIndexes.indexOf(0)];
        missingShardBuffer = Buffer.alloc(parity.length);
        for (let i = 0; i < parity.length; i++) {
          missingShardBuffer[i] = parity[i] ^ data0[i];
        }
      }
